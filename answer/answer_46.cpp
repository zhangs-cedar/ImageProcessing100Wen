...
cv::Mat non_maximum_suppression(cv::Mat angle, cv::Mat edge) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if (fmax(fmax(edge.at<uchar>(y, x), edge.at<uchar>(y + dy1, x + dx1)),
               edge.at<uchar>(y + dy2, x + dx2)) == edge.at<uchar>(y, x)) {
      ...
      // if LT < pixel < HT
        ...
        for (int dy = -1; dy < 2; dy++) {
          for (int dx = -1; dx < 2; dx++) {
            ...
            if (edge.at<uchar>(fmin(fmax(y + dy, 0), 255),
                               fmin(fmax(x + dx, 0), 255)) >= HT) {
              _edge.at<uchar>(y, x) = 255;
            }
          }
        }
...
cv::Mat Hough_line(cv::Mat img) {
  ...
  // hough inverse
  cv::Mat out = Hough_inverse(hough_table, img);
...
int main(int argc, const char *argv[]) {
  ...
  // Hough line detection
  cv::Mat hough = Hough_line(img);

  // cv::imwrite("out.jpg", out);
  cv::imshow("answer(hough)", hough);
  cv::waitKey(0);
